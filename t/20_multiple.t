...
My::Test->spawn( alias=>'dnotify', delay=>2 );
My::Test2->spawn( alias=>'dnotify' );
...
sub _start
{
    ...
    $self->{delay} = poe->kernel->delay_set( start => $self->{delay} )
                if $self->{delay};
}
...
sub spawn
{
    ...
    POE::Session::PlainCall->create(
                    package   => $package,
                    ctor_args => [ \%init ],
                    states    => [ qw( _start _stop notify1 ) ]
                );
}
...
{
    my( $self, $change, $path ) = @_;
    is( $path, $self->{dir}, "Change number $self->{N} in $self->{dir}" );
    $self->{N}--;
